jQuery(document).ready(function() {
    ...
    jQuery('#ds-avatar').click(function() {
        jQuery("form#comment-form div.author-info").slideToggle("slow");
    });

    jQuery('#textarea').keypress(function(e) {
        if (e.ctrlKey && e.which == 13 || e.which == 10) {
            jQuery('.ds-post-button').click();
        }
    });

    jQuery('.ds-post-button').click(function() {
        if (jQuery('#mail').val() == '' || jQuery('#author').val() == '')
            jQuery("form#comment-form div.author-info").slideDown("slow");
    });

    jQuery('.ds-add-emote').click(function() {
        var $container = jQuery('#ds-smilies-tooltip');
        var offset = jQuery(this).position();            
            $container.css('top',top);
            $container.css('left',offset.left);
            $container.css('width','350');
